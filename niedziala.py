...
# Function to adjust Date/Time column
def adjust_datetime(csv_file_path):
    # Read the CSV file
    ...
    # Subtract 30 minutes from each datetime
    df['Date/Time'] = df['Date/Time'] + pd.Timedelta(minutes=1)

    # Save the updated DataFrame back to the same CSV file
    df.to_csv(csv_file_path, index=False)
    print(f"File saved to {csv_file_path}")

# Adjust the 'Date/Time' column in positions.csv
adjust_datetime('positions.csv')
...
# Format daty używany w twojej liście
date_format = "%Y-%m-%d %H:%M"

# Lista do przechowywania zaktualizowanych dat
updated_dates = []

# Odjęcie 30 minut od każdej daty
for date_str in positions_dates:
    # Konwersja stringa na obiekt datetime
    date_obj = datetime.strptime(date_str, date_format)
    # Odjęcie 30 minut
    updated_date_obj = date_obj + timedelta(minutes=30)
    # Konwersja zaktualizowanej daty z powrotem na string
    updated_date_str = updated_date_obj.strftime(date_format)
    # Dodanie zaktualizowanej daty do listy
    updated_dates.append(updated_date_str)

# Wyświetlenie zaktualizowanej listy dat
print(updated_dates)
prices_dates = updated_dates
        ...
        if found_entry:
            if positions_types[i] == 'Entry Short':
                ...
                for y in range(x, len(prices_dates)):
                    ...
                    print("testing:", close_price_date)
